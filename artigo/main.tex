...
e JBI \citep{Peters2020}. A busca foi concentrada na base de dados primária PubMed/MEDLINE (filtros: 2014-presente, inglês/espanhol e texto completo gratuito) e complementada por busca manual via \textit{snowballing}. O corpus final compreende 24~artigos selecionados a partir de 786~registros da busca eletrônica primária (após remoção de 27 duplicatas) e 6 estudos adicionados manualmente por busca ativa, organizados em quatro eixos temáticos: (A)~NLP clínico e extração de entidades
...
\begin{itemize}
  \item \textbf{IC1 (Foco Temático):} O estudo aplica tecnologias (\textit{Speech Analytics}, NLP, ASR) para transcrição clínica e extração de dados médicos não estruturados, OU analisa/mensura os fundamentos da comunicação profissional-paciente.
  \item \textbf{IC2 (Fonte de Dados):} O estudo foca em interações baseadas em texto livre de prontuários (\textit{free-text}), diálogos clínicos falados (consultas), geração de documentação automatizada (escribas digitais/notas SOAP) ou marcos teóricos da tomada de decisão compartilhada.
  \item \textbf{IC3 (Idioma e Período):} Texto completo disponível em inglês, português ou espanhol, publicado no período de janeiro de 2014 a 2026.
  \item \textbf{IC4 (Metodologia):} Apresenta desenvolvimento original, testes de viabilidade empíricos, validação de algoritmos, ou trata-se de revisão sistemática/scoping review anterior sobre o tema.
\end{itemize}

\subsubsection{Critérios de exclusão}
\begin{itemize}
  \item \textbf{EC1 (Foco Tecnológico):} O estudo não aborda o uso de processamento de linguagem natural (NLP), reconhecimento automático de fala (ASR) ou análise de áudio de interações clínicas.
  \item \textbf{EC2 (Foco Comunicacional):} O estudo descreve sistemas de transcrição ou extração sem analisar a interação, a qualidade da comunicação ou o impacto na relação profissional-paciente.
  \item \textbf{EC3 (Tipo de Estudo/Dados):} Foco exclusivo em diagnóstico por voz de patologias físicas ou psiquiátricas sem análise da interação dialógica.
  \item \textbf{EC4 (Tipo de Publicação):} Artigos de opinião, comentários, editoriais, resumos estendidos de conferências ou livros.
  \item \textbf{EC5 (Acesso aos Dados):} Falta de acesso ao texto completo gratuito (\textit{paywall}) e impossibilidade de obtenção por vias institucionais.
  \item \textbf{EC6 (Desenho Metodológico):} Estudos que não descrevem a metodologia de coleta, processamento de dados ou validação técnica.
  \item \textbf{EC7 (Desenho do Cenário):} Estudos focados exclusivamente em interações não clínicas ou em contextos simulados sem potencial de transferência prática demonstrado.
\end{itemize}

\subsection{Fontes de informação e estratégia de busca}

A busca bibliográfica foi conduzida de forma independente por três pesquisadores diferentes, cada um operando múltiplas estratégias de busca para cobrir os eixos temáticos da revisão. A base de dados primária utilizada foi o PubMed/MEDLINE. Foram aplicados os seguintes filtros de busca: período de publicação de janeiro de 2014 ao presente (2026), artigos publicados nos idiomas inglês e espanhol, e disponibilidade de texto completo gratuito (\textit{free full text}).

A utilização do filtro de texto completo gratuito (\textit{free full text}) decorreu de limitações operacionais iniciais de acesso e credenciamento institucional para recuperação de artigos em sua integridade.
...
concordância inter-examinadores avaliada pelo coeficiente kappa de Cohen ($\kappa = 0{,}78$). Desacordos
...
As buscas na base de dados primária (PubMed/MEDLINE) recuperaram 786~registros. Após remoção de 27~duplicatas, 759~registros foram triados por título e resumo (com índice de concordância inter-examinadores $\kappa = 0{,}78$), resultando em 35~artigos selecionados para avaliação de texto completo. Desses, 17~foram excluídos após leitura integral por não atenderem aos critérios de elegibilidade definidos, distribuídos da seguinte forma: 3~artigos por EC1 (foco tecnológico inadequado), 2~por EC2 (ausência de análise da interação ou impacto na comunicação), 3~por EC3 (foco exclusivo em diagnóstico por voz de patologias físicas), 2~por EC4 (tipo de publicação inadequado), 2~por EC5 (falta de acesso ao texto completo), 3~por EC6 (desenho metodológico omisso) e 2~por EC7 (interações não clínicas ou cenários simulados sem transferibilidade). Adicionalmente, 6~artigos identificados via \textit{snowballing} (rastreamento retroativo de referências) foram avaliados de forma independente utilizando os mesmos critérios; todos atenderam aos requisitos e nenhum foi excluído. Ao final, 24~estudos foram incluídos na síntese qualitativa (18~da busca eletrônica na base primária e 6~provenientes do rastreamento manual). O fluxograma PRISMA-ScR detalhando todo o processo está apresentado na Fig.~\ref{fig:prisma}.
...
\multicolumn{7}{l}{\footnotesize
  \textsuperscript{‡}Preprint/não peer-reviewed (medRxiv) — IC2 a confirmar com política editorial do IJMI.} \\
\end{longtable}
...
%% REVISAR [Fabrício]: verificar se os dados reportados correspondem
...
Primeiro, embora a busca eletrônica tenha se concentrado no PubMed/MEDLINE como base de dados primária, a inclusão de artigos adicionais via \textit{snowballing} (incluindo revisões sistemáticas relevantes e estudos de referência) visou mitigar a perda de artigos essenciais; contudo, a ausência de buscas automatizadas em outras bases eletrônicas indexadas (como Scopus e Web of Science) permanece como uma limitação para a generalização completa da literatura.
...
Terceiro, a seleção de filtros de busca na base primária, como a exigência de texto completo gratuito (\textit{free full text}), decorreu de limitações operacionais de acesso e credenciamento no momento da busca, o que pode ter excluído estudos relevantes protegidos por paywall ou sob restrição de assinaturas institucionais; tal viés foi mitigado pelo uso de \textit{snowballing} para resgatar estudos-chave sob paywall identificados na literatura.
Quarto, a heterogeneidade de tipo de estudo, que inclui revisões sistemáticas, estudos empíricos, preprints e modelos conceituais, impede a realização de uma síntese quantitativa ou meta-análise estruturada, limitando também as comparações diretas de qualidade metodológica entre as abordagens.
...
%% REVISAR [Fabrício]: conferir consistência com a Discussão; ajustar
