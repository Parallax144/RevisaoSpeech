...
%% REVISAR [Rafael + Murilo]: substituir [N] pelos números finais após Gate 2
%% ----------------------------------------------------------------
\begin{abstract}
\textbf{Contexto:}
...
escopo. Falcetta et al. \citeyearpar{Falcetta2023} identificaram ausência
de validação prospectiva em sistemas de documentação automática de
interações clínicas. Ng et al. \citeyearpar{Ng2025} avaliaram o desempenho
técnico de sistemas de ASR para documentação clínica, reportando WER
entre 8,7\% e mais de 50\% conforme o contexto. Alabbad e Alhoshan
\citeyearpar{Alabbad2026} mapearam aplicações de ASR na saúde na era
pós-LLM. Albert et al. \citeyearpar{Albert2024} realizaram uma revisão de
...
identificação de registros adicionais. As buscas foram executadas entre 28 de maio de~2026 e 11 de junho de~2026.
...
%% NOTA METODOLÓGICA: κ calculado após triagem dupla-cega no Rayyan (F.E.S. e M.M.).
%% Workflow: exportar matriz de concordância do Rayyan → R: irr::kappa2(ratings, weight="unweighted")
%% REVISAR [Fabricio + Murilo]: inserir κ = [valor] e ns de cada etapa após Gate 2
...
%% CONTAGENS — preencher [N] abaixo com valores exatos após Gate 2
%% PubMed: 8 (exato, API NCBI jun/2026)
%% Scopus: ~275 · IEEE: ~125 · ACM: ~50 · WoS: ~200 · arXiv: ~28  (estimados; requerem acesso USF)
%% REVISAR [Murilo]: substituir estimados pelos exatos e preencher todos os [N] no texto abaixo

As buscas nas seis bases recuperaram aproximadamente 686~registros no total
(PubMed/MEDLINE:~8\textsuperscript{*}; Scopus:~$\approx$275;
IEEE~Xplore:~$\approx$125; ACM~Digital~Library:~$\approx$50;
Web~of~Science:~$\approx$200; arXiv:~$\approx$28).
Após remoção de duplicatas, [N]~registros foram triados por título e
resumo. A concordância entre os dois revisores primários foi $\kappa = $[VALOR]
(F.E.S. e M.M.). Após avaliação de texto completo de [N]~registros,
...
\footnotetext[1]{Contagem exata via API NCBI E-utilities (jun/2026).
Demais valores são estimativas preliminares a serem substituídas pelas
contagens obtidas com acesso institucional USF.}
...
%% REVISAR [Murilo]: checar IC/EC para cada linha antes da submissão.
%%   † = fora do período ≥2018 (discutir inclusão como referência fundacional)
%%   ‡ = preprint / não peer-reviewed (verificar IC2)
%%   n e período finais aguardam triagem formal (Gate 2).

Os 22~estudos do corpus inicial abrangem o período 2014--2025
(Tabela~\ref{tab:estudos}).

\begin{longtable}{p{0.65cm} p{2.4cm} p{1.55cm} p{0.55cm} p{2.2cm} p{2.35cm} p{1.4cm}}
\caption{Características dos estudos do corpus inicial (BaseCitacoes.md).}
\label{tab:estudos}\\
\toprule
...
S10 & Aleksić et al., 2017\textsuperscript{†}  & Empírico & B & Resumo de EHR & Dashboard para doença crônica & Sérvia  \\
...
S14 & Shao et al., 2025      & Empírico     & C & Escala DPCQ (14 itens)     & Qualidade da comunicação médico-paciente & China       \\
S15 & Muscat et al., 2020    & Framework    & C & Tomada de decisão compartilhada & Literacia em saúde; engajamento & Austrália  \\
S16 & Kelley et al., 2014\textsuperscript{†}  & Meta-análise & C & --  & Relação clínica → desfechos objetivos & EUA      \\
S17 & Rucinski et al., 2022  & Rev.\ sist.  & C & Métricas de adesão         & Adesão bilateral em ortopedia          & EUA           \\
S18 & McCabe \& Healey, 2018 & Empírico     & C & Análise de conversa        & Autorreparo como preditor de adesão    & RU            \\
S19 & Alkhamees \& Alasqah, 2023 & Rev.\ integr. & C & --               & Barreiras interculturais na comunicação & Arábia S.   \\
%% ---- Eixo D: Biomarcadores vocais e análise multimodal ----
S20 & Fagherazzi et al., 2021 & Framework   & D & Pipeline vocal (acústica+NLP) & Biomarcadores vocais de saúde e emoção & Luxemburgo  \\
S21 & Albert et al., 2024    & Scoping rev.\textsuperscript{‡} & D & IA, Speech, NLP & Comunicação clínico-paciente via IA & França  \\
S22 & Imel et al., 2022      & Relatório    & D & Redes neurais sequenciais  & Valência emocional; concordância humana & EUA          \\
\midrule
\multicolumn{7}{l}{\footnotesize
  \textsuperscript{†}Fora do período IC ($\geq$2018) — confirmar elegibilidade.
  \quad\textsuperscript{‡}Preprint, não peer-reviewed — confirmar IC2.} \\
\end{longtable}
...
%% REVISAR [Murilo]: verificar se os dados reportados correspondem
%% aos artigos originais antes da submissão. Todas as afirmações
%% estão ancoradas na BaseCitacoes.md.
...
identificada -- apontada por \citet{Navarro2023} e \citet{Bazoge2023}
...
%% REVISAR [Fabricio]: verificar se os dados reportados correspondem
%% aos artigos originais antes da submissão. Aleksić 2017 é referência
%% fundacional (fora do período ≥2018) — confirmar inclusão na síntese.
%% Krishna 2021 e Schloss 2020 são preprints — confirmar IC2.
...
das iniciativas restrita a provas de conceito em ambiente laboratorial.
...
%% REVISAR [Lucas]: verificar se os dados reportados correspondem
%% aos artigos originais antes da submissão. Kelley 2014 é referência
%% fundacional fora do período ≥2018 — confirmar elegibilidade.
...
disparidades idiomáticas e estilos autoritários de condução da
...
%% REVISAR [Fabricio]: verificar se os dados reportados correspondem
%% aos artigos originais. Albert 2024 é preprint (medRxiv) — confirmar IC2.
...
mapearam valências emocionais em áudios de interações médico-paciente
por meio de redes neurais sequenciais, alcançando concordância com
juízes humanos treinados. O estudo demonstra que marcadores emocionais
da interação verbal podem ser identificados automaticamente com
confiabilidade comparável à avaliação humana, abrindo caminho para a
mensuração de empatia e qualidade do atendimento em larga escala.
...
de desenvolvimento: os fundamentos técnicos existem
\citep{Fagherazzi2021} e há evidência de que marcadores emocionais
podem ser identificados com confiabilidade automática
\citep{Imel2022}, mas a integração multimodal acústico-semântica
aplicada à comunicação clínica permanece incipiente
\citep{Albert2024}. Essa lacuna adquire contorno clínico preciso
...
%% REVISAR [Murilo + Lucas]: conferir argumentação e adicionar perspectiva de
%%   cada coautor; atualizar após Gate 2 (contagens finais do PRISMA).
...
Estudos de NLP como Bazoge et al.\ \citeyearpar{Bazoge2023} e
Wang et al.\ \citeyearpar{Wang2018} empregam pipelines de extração de
entidades sobre texto retroativo de prontuários; estudos de ASR como
van~Buchem et al.\ \citeyearpar{vanBuchem2021} e
Klusty et al.\ \citeyearpar{Klusty2025} medem a acurácia de transcrição
e a redução do tempo de documentação; estudos de comunicação como
Shao et al.\ \citeyearpar{Shao2025} e Kelley et al.\ \citeyearpar{Kelley2014}
...
Krishna et al.\ \citeyearpar{Krishna2021} e
Schloss \& Konam \citeyearpar{Schloss2020} classificam enunciados pela
estrutura SOAP, não pela qualidade relacional do encontro.
Abbasian et al.\ \citeyearpar{Abbasian2024} avaliam a eficácia de
conversas geradas por IA generativa, não o comportamento comunicativo do
profissional de saúde em contexto real.
Shao et al.\ \citeyearpar{Shao2025} e McCabe \& Healey
\citeyearpar{McCabe2018} descrevem dimensões relevantes de qualidade
...
A revisão de Albert et al.\ \citeyearpar{Albert2024} é a que mais
...
Porto \citeyearpar{Porto2019} oferece um referencial semiológico
...
Baena~Neto \& Becerra~Sablón \citeyearpar{BaenaNeto2026} propõem
...
Esta revisão apresenta quatro limitações centrais.
Primeiro, o corpus analisado constitui uma fase de mapeamento inicial
(\emph{BaseCitacoes.md}, $n = 22$), pendente de complementação pelas
buscas com acesso institucional nas cinco bases eletrônicas restantes
(Scopus, IEEE~Xplore, ACM~DL, Web of Science e arXiv), responsáveis por
$\approx$678 dos registros estimados; os achados devem ser interpretados
como mapeamento exploratório, não como síntese sistemática definitiva.
Segundo, há viés de idioma: a literatura mapeada é predominantemente em
inglês, com sub-representação de estudos conduzidos no contexto de saúde
brasileiro e em Português, o que pode subestimar iniciativas locais
relevantes para o SUS.
Terceiro, a string de busca com três blocos combinados por AND é
restritiva — evidenciada pelos oito resultados exatos no PubMed —,
recomendando-se testar pares de blocos antes de Gate~2 para ampliar o
corpus sem perda de precisão temática.
Quarto, a heterogeneidade de tipo de estudo — revisões sistemáticas,
estudos empíricos, preprints e modelos conceituais — impede síntese
quantitativa e limita comparações diretas de qualidade metodológica;
o instrumento Dybà \& Dingsøyr aplica-se ao subconjunto empírico e
sua pontuação deve ser interpretada com cautela para revisões e
...
Do ponto de vista técnico, Klusty et al.\ \citeyearpar{Klusty2025}
...
propostos por Floridi et al.\ \citeyearpar{Floridi2018} — beneficência,
...
%% REVISAR [Fabricio]: conferir consistência com a Discussão; ajustar
%%   após Gate 2 se os números finais do PRISMA alterarem o argumento.
...
O protocolo metodológico descrito em Baena~Neto \& Becerra~Sablón
\citeyearpar{BaenaNeto2026} representa uma resposta direta a esse
...
%% COMPLETAR [Rafael]: inserir DOI ou URL do repositório (OSF / Zenodo /
%%   GitHub) após submissão, se optarem por depósito público.
...
(\textit{disponível mediante aceitação});
...
%% COMPLETAR: adicionar qualquer outra ferramenta de IA usada (ex.: DeepL,
%%   ChatGPT, Gemini) conforme exigência da Política GenAI da Elsevier.
